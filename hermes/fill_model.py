...
def conservative_paper_fill(
    ...
    *,
    ...
    Returns (filled_usd, fill_price, slippage_bps, note):
      ...
      - fill_price = VWAP over the filled notional, worsened by the
        near-the-money penalty, never better than the limit;
      - slippage_bps measured vs mid when known.
    """
    ...
    # VWAP walk for the filled notional.
    remaining = filled
    cost = 0.0
    shares = 0.0
    for price, size in asks:
        ...
        if remaining <= 1e-9:
            break
    vwap = cost / shares if shares > 0 else float(asks[0][0])

    penalty = near_money_penalty_bps(mid if mid is not None else vwap)
    px = vwap * (1.0 + penalty / 10_000.0)
